...
export async function generateMetadata({
  ...
  try {
    ...
    return { title: "Conteúdo — Gustavo Trotta" };
  }
}
...
const ptComponents: PortableTextComponents = {
  block: {
    ...
    blockquote: ({ children }) => (
      <blockquote className="mt-7 border-l-2 border-gold-500 pl-6 font-serif text-[1.15rem] italic leading-relaxed text-ink-700">
        {children}
      </blockquote>
    ),
  },
...
export default async function PublicArticlePage({
  ...
  try {
    ...
    console.error("[conteudo/slug] fetch error:", err);
  }
      ...
      <Container>
        <article className="mx-auto max-w-3xl">
          ...
          <nav className="text-[0.7rem] uppercase tracking-wider2 text-muted-500">
            <Link href="/conteudo" className="hover:text-ink-900">
              Conteúdo
            </Link>
            {article.category && (
              <>
                ...
                <Link
                  href={`/conteudo?categoria=${article.category.slug}`}
                  ...
                  <>
                    ...
                    <ol className="mt-3 space-y-3">
                      {article.keyInsights.map((insight, i) => (
                        <li
                          key={i}
                          className="flex gap-4 text-[1rem] leading-relaxed text-ink-800"
                        >
